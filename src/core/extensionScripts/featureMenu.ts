...
function createRow(
  ...
  const hA = document.createElement(isHeader ? 'th' : 'td');
  const hB = document.createElement(isHeader ? 'th' : 'td');
  const hC = document.createElement(isHeader ? 'th' : 'td');
...
async function _appendFeature(e: AFeature, featureTable: HTMLTableElement) {
  ...
  enabledCheckbox.onclick = () => {
    featureStore.isFlagEnabledAsync(e.name).then((isEnabled) => {
