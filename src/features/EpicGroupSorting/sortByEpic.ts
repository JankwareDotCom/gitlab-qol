...
import { FeatureOptionList } from '../../core/FeatureOptionService/IFeatureOption';
...
export default class EpicGroupSorting extends AFeature {
  ...
  injectFeature(): void {
    ...
    featureMenuDiv.setAttribute(
      'class',
      'board-swimlanes-toggle-wrapper gl-md-display-flex gl-align-items-center gl-ml-3',
    );

    featureMenuSpanLabel.setAttribute(
      'class',
      'board-swimlanes-toggle-text gl-white-space-nowrap gl-font-weight-bold gl-line-height-normal',
    );
    ...
    const checker = setInterval(() => {
      const menuTargets = document.getElementsByClassName(filterContainerSelector);
  ...
  _applyAutomaticSort() {
    featureStore.getOptionValueAsync<string>(this.name, 'Automatic Sort').then((val) => {
      const selectBox = this._getSortByEpicSelectElement();
  ...
  _addGroupByListener(): void {
    ...
    const observer = new MutationObserver(() => {
      ...
      const sbeSelect = this._getSortByEpicSelectElement();
      ...
      const sbe = this._getSortByEpicDiv();
      if (sbe !== null) {
  ...
  _getSortByEpicDiv(): HTMLElement | null {
    const sbe = document.getElementById('sortByEpic');
  ...
  _getSortByEpicSelectElement(): HTMLSelectElement | null {
    ...
    const sbeSelect = sbe.querySelector('select');
  ...
  _getEpicLaneContainer(): Element {
    ...
    const epicLaneContainer = document
  ...
  _getEpicLanes(epicLaneContainer: Element): Element[] {
    ...
    const epicLanes = epicLaneContainer.getElementsByClassName(singleLaneSelectorClass);

    if (epicLanes === undefined || epicLanes.length == 0) {
      throw new Error('Epic lanes not found');
    }

    const epicLaneArr = Array<Element>().slice.call(epicLanes);
    return epicLaneArr;
  }

  _applySort(
    sortMethod: (epicLaneArr: Element[], service: EpicGroupSorting) => Promise<Element[]>,
    service: EpicGroupSorting,
    inverse: boolean,
  ) {
  ...
  _sortByDefault(epicLaneArr: Element[]): Promise<Element[]> {
    epicLaneArr = _ogLaneArray;
    return new Promise((resolve) => resolve(epicLaneArr));
  }

  _sortByTitle(epicLaneArr: Element[]): Promise<Element[]> {
    const singleLaneTitleSelector = 'div div h4';
    const sorted = epicLaneArr.sort((a, b) => {
      return (a.querySelector(singleLaneTitleSelector)?.innerHTML.toLowerCase() ?? '').localeCompare(
        b.querySelector(singleLaneTitleSelector)?.innerHTML.toLowerCase() ?? '',
      );
    });
  ...
  async _sortByIssueCount(epicLaneArr: Element[], service: EpicGroupSorting): Promise<Element[]> {
    ...
    if (!service._issueCountsLoaded) {
      const readyInt = setInterval(() => {
        ...
        epicLaneArr.forEach((e) => {
          ...
          const issueCount = parseInt(issueSelector?.innerHTML ?? '');
        ...
        if (localReady) {
          ...
          const selectBox = service._getSortByEpicSelectElement();
    ...
    const sorted = epicLaneArr.sort((a, b) => {
      return (a.querySelector(singleLaneIssueSelector)?.innerHTML.toLowerCase() ?? '').localeCompare(
        b.querySelector(singleLaneIssueSelector)?.innerHTML.toLowerCase() ?? '',
        undefined,
...
export class AutomaticSort extends FeatureOptionList<string> {
  constructor() {
    super('Automatic Sort', 'Automatically Sort using the defined method when board loads', 'Default', {
      Default: 'Default',
      Title: 'Title',
      '# of Issues': '# of Issues',
    });
